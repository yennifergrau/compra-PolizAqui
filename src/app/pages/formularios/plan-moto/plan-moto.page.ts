...
public onCiudadTI(event: Event): void {
 ...
 public getAnoDescription(): string {
  ...
  try {
    const response = await (await this.mundialService.marcaMundialMoto(data)).toPromise();
  ...
  public getDescripcionModelo(): string {
    ...
    public async selectVersion(version:Version): Promise<void> {
  ...
  ngOnInit() {
    ...
    const correoUsuario : any = jwtDecode(emailInfo.infoUser)
    this.email = correoUsuario.email
